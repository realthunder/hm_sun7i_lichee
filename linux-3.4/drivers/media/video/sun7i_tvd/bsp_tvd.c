...
void tvd_comb_3d(__u32 enable, __u32 size, __u32 addr1, __u32 addr2, __u32 channel_index)
{
	__u32 reg_val;

	if (enable) {
		REG_WR32(addr_base + 0x0510, (0x04040001 | (channel_index << 4)));

		//0x514:TVD 3D comb start address H register
		//0x518:TVD 3D comb start address L register
		REG_WR32(addr_base + 0x0514, addr1);//addr1
		REG_WR32(addr_base + 0x0518, addr2);//addr2
		//size = 0x200000
		//REG_WR32(addr_base + 0x051c, 0x00200000);//size
		REG_WR32(addr_base + 0x051c, size);//size
		//REG_WR32(addr_base + 0x000c, 0x00000010); //YC sep

		//COMB2D only
		//this bit will enable TVIN to work only with2d Yc
		//separation mode ,with this mode enable, no frame
		//buffer is required
		//default:0
		reg_val = REG_RD32(addr_base + 0x0070);
		reg_val &= ~(1 << 6);
		REG_WR32(addr_base + 0x0070, reg_val);

		//mode 3D enable
		reg_val = REG_RD32(addr_base + 0x0000);
		reg_val |= (1 << 4);
		REG_WR32(addr_base + 0x0000, reg_val);
	} else {
		REG_WR32(addr_base + 0x000c, 0x00000000); //YC sep
		reg_val = REG_RD32(addr_base + 0x0070);
		reg_val |= (1 << 6);
		REG_WR32(addr_base + 0x0070, reg_val);
...
//write 0x134,0x234,0x334,0x434 register bit0-11 set crop line length
void TVD_set_width(__u32 id,__u32 w)
{
	__u32 reg_val;
	reg_val = REG_RD32(addr_base + 0x134+0x100*id);
	reg_val &= ~(0xfff<<0);//bit0-11
	reg_val |= ((w>720)?720:w)<<0;
	REG_WR32(addr_base+0x0134+0x100*id, reg_val);
}

//write 0x138,0x238,0x338,0x438 register bit0-11 set jump line length
