...
//#define TVD_SYS_DEBUG
#ifdef TVD_SYS_DEBUG
#define sys_dbg(x, arg...) printk(KERN_INFO"[TVD_SYS_DBG]"x, ##arg)
#else
#define sys_dbg(x, arg...)
#endif

#ifdef CONFIG_HAS_EARLYSUSPEND
static void tvd_early_suspend(struct early_suspend *h);
static void tvd_late_resume(struct early_suspend *h);
#endif
...
static struct fmt * get_format(struct v4l2_format *format)
{
	...
	for (k = 0; k < ARRAY_SIZE(formats); k++) {
		...
		{
			__inf("fmt->fourcc = %d, fmt->width=%d, fmt->height = %d\n", fmt->fourcc, fmt->width, fmt->height);
			break;
		}
	}
...
#ifdef TVD_3D_COMP
//#define TVD_3D_COMP_BUFFER_SIZE (0x200000)
static void tvd_3d_comp_mem_request(int size, __u32 *phy_addr)
{
	__u32 actual_size = 0;
	__u32 address = 0;

	actual_size = PAGE_ALIGN(TVD_3D_COMP_BUFFER_SIZE);
	address = sunxi_mem_alloc(actual_size);
	if (!address) {
		__err("sunxi_mem_alloc faild\n");
	}
...
static void tvd_3d_comp_mem_free(__u32 phy_addr)
{
	sunxi_mem_free(phy_addr);
}
#endif
...
static int tvd_clk_init(struct tvd_dev *dev,int interface)
{
	...
	module_clk_src=clk_get(NULL,"video_pll1"); //can select video_pll0 or video_pll1
...
#ifdef TVD_3D_COMP
	tvd_3d_comp_mem_request(TVD_3D_COMP_BUFFER_SIZE, &dev->addr1);
	tvd_3d_comp_mem_request(TVD_3D_COMP_BUFFER_SIZE, &dev->addr2);
#endif
...
#ifdef TVD_3D_COMP
	tvd_3d_comp_mem_free(dev->addr1);
	tvd_3d_comp_mem_free(dev->addr2);
#endif
	return ret;
}
...
static int vidioc_s_fmt_vid_cap(struct file *file, void *priv,struct v4l2_format *format)
{
	...
	dev->height             = format->fmt.pix.height;
        ...
        if (!(dev->height%480)) {
		dev->column = dev->height / 480;
		dev->system = 0;//ntsc
	} else if (!(dev->height%576)) {
		dev->column = dev->height / 576;
		dev->system = 1;//pal
	}

        if (!(dev->width%720)) {
		dev->row = dev->width / 720;
		dev->format = 0; //non mb 
		width = 720;
	} else if (!(dev->width%704)) {
		dev->row = dev->width / 704;
		dev->format = 1;//mb
		width = 704;
	}

	if ((dev->format == TVD_MB_YUV420) && (dev->column > 1)) {
		//高度剪掉32个像素点，以适配448%2%32=0
	...
	__inf("width=%d\n",dev->width);
	__inf("height=%d\n",dev->height);
	//other info from sys_config.fex
        TVD_config(dev->interface, dev->system);

#ifdef TVD_3D_COMP
	if (dev->interface == 0) {//tvd_cvbs
		for(i=0; i<4; i++) {
			if(dev->channel_index[i])
				tvd_comb_3d(1, TVD_3D_COMP_BUFFER_SIZE, dev->addr1, dev->addr2, i);
		}
	}
#endif
	...
	for(i=0;i<4;i++){
		if(dev->channel_index[i]){
			TVD_set_fmt(i, dev->format);
			TVD_set_width(i, (dev->width / dev->row));
			if(dev->interface==2)
				TVD_set_height(i, (dev->system?576:480));
			else
				TVD_set_height(i, (dev->system?576:480)/2);
			TVD_set_width_jump(i, dev->width);

			if (dev->format == TVD_PL_YUV420) {
				dev->channel_offset_y[i] = ((dev->channel_index[i]-1)%dev->row) * width
					+ ((dev->channel_index[i]-1)/dev->row)*dev->row * width * height;

				dev->channel_offset_c[i] = ((dev->channel_index[i]-1)%dev->row) * width
					+ ((dev->channel_index[i]-1)/dev->row)*dev->row * width * height/2;
				...
				dev->channel_offset_c[i] = dev->channel_offset_y[i];
					...
					dev->channel_offset_y[i] = ((dev->channel_index[i]-1)%dev->row)* width * 32
						+ ((dev->channel_index[i]-1)/dev->row)*dev->row* width * height;
					dev->channel_offset_c[i] = ((dev->channel_index[i]-1)%dev->row)* width * 32
						+ ((dev->channel_index[i]-1)/dev->row)*dev->row * width * height/2;
			}
		}			
	}
...
static int vidioc_s_fmt_type_private(struct file *file, void *priv,struct v4l2_format *format)
{
	...
	dev->height             = dev->column*height;
	tvd_fmt.width           = dev->width;
	tvd_fmt.height          = dev->height;
	tvd_fmt.output_fmt	= dev->format;
	dev->fmt                = &tvd_fmt;
	...
	for(i=0;i<4;i++){
		if(dev->channel_index[i]){
			...
			TVD_set_width(i, (dev->width / dev->row));
			if(dev->interface==2)
				TVD_set_height(i, height);
			else
				TVD_set_height(i, height/2);
			...
			__dbg("channel_offset_y[%d]=%d\n", i, dev->channel_offset_y[i]);
			__dbg("channel_offset_c[%d]=%d\n", i, dev->channel_offset_c[i]);
		}			
	}
...
static int fetch_config(struct tvd_dev *dev)
{
	script_item_u   val;
	script_item_value_type_e  type;
	
	type = script_get_item("tvin_para","tvin_system", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->system = 0;//default NTSC
		__inf("fetch tvin_system from sys_config failed, default NTSC\n");
	} else {
		dev->system = val.val;
	}

	type = script_get_item("tvin_para","tvin_interface", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->interface = 0;//default CVBS
		__inf("fetch tvin_interface from sys_config failed, default =0\n");
	} else {
		dev->interface = val.val;
	}

	type = script_get_item("tvin_para","tvin_format", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->format = 1;//default MB_YUV420
		__inf("fetch tvin_format from sys_config failed, default =1\n");
	} else {
		dev->format = val.val;
	}

	type = script_get_item("tvin_para","tvin_channel0_en", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[0] = 1;//default only open channel0
		__inf("fetch tvin_channel0_en from sys_config failed, default =1\n");
	} else {
		dev->channel_index[0] = val.val;
	}

	type = script_get_item("tvin_para","tvin_channel1_en", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[1] = 0;//default only open channel0
		__inf("fetch tvin_channel1_en from sys_config failed, default =0\n");
	} else {
		dev->channel_index[1] = val.val;
	}

	type = script_get_item("tvin_para","tvin_channel2_en", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[2] = 0;//default only open channel0
		__inf("fetch tvin_channel2_en from sys_config failed, default =0\n");
	} else {
		dev->channel_index[2] = val.val;
	}

	type = script_get_item("tvin_para","tvin_channel3_en", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->channel_index[3] = 0;//default only open channel0
		__inf("fetch tvin_channel3_en from sys_config failed, default =0\n");
	} else {
		dev->channel_index[3] = val.val;
	}

	type = script_get_item("tvin_para","tvin_row", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->row = 1;//default
		__inf("fetch tvin_row from sys_config failed, default =1\n");
	} else {
		dev->row = val.val;
	}

	type = script_get_item("tvin_para","tvin_column", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		dev->column = 1;//default
		__inf("fetch tvin_column from sys_config failed, default =1\n");
	} else {
		dev->column = val.val;
	}
...
static int tvd_probe(struct platform_device *pdev)
{
	...
	*vfd = device;
	...
	//init_waitqueue_head(&dev->vidq.wq);
        ...
        dev->early_suspend.suspend = tvd_early_suspend;
        dev->early_suspend.resume = tvd_late_resume;
...
#ifdef CONFIG_PM
#ifdef CONFIG_HAS_EARLYSUSPEND
void tvd_early_suspend(struct early_suspend *h)
{
	__inf("tvd early suspend enter\n");
}

void tvd_late_resume(struct early_suspend *h)
{
	struct tvd_dev *dev;

	dev = container_of(h, struct tvd_dev, early_suspend);
	__inf("tvd late resume enter\n");
	if (dev->opened==1) {
		if (tvd_clk_init(dev,dev->interface))
			__err("clock init fail!\n");
	}
...
#ifdef TVD_SYS_DEBUG
struct device tvd_dbg_device;
static ssize_t tvd_sys_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
        return 0;
}

static ssize_t tvd_sys_store(struct device *dev,
...
static DEVICE_ATTR(tvd, S_IWUSR | S_IRUGO, tvd_sys_show, tvd_sys_store);

static int tvd_sys_dbg(void)
{
        int ret;

	dev_set_name(&tvd_dbg_device, "tvd");

	if (device_register(&tvd_dbg_device))
                sys_dbg("error device_register()\n");

	ret = device_create_file(&tvd_dbg_device, &dev_attr_tvd);
	if (ret)
                sys_dbg("device_create_file error\n");

        sys_dbg("tvd device register ok and device_create_file ok\n");
        return 0;
}
#endif
...
static int __init tvd_init(void)
{
	...
	__dbg("%s\n", __FUNCTION__);
	type = script_get_item("tvin_para","tvin_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		__err("fetch tvin_used from sys_config.fex faild\n");
		return -1;
	} else {
		tvin_used=val.val;
	}
	...
	if (!tvin_used)
	{
		__err("tvin_used=0,tvin driver is not enabled!\n");
		return 0;
	}
...
static void __exit tvd_exit(void)
{
	...
	type = script_get_item("tvin_para","tvin_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		__err("fetch tvin_used from sys_config.fex failed\n");
		return;
	} else {
		tvin_used=val.val;
	}
...
MODULE_AUTHOR("yaowenjun@allwinnertech.com");
MODULE_LICENSE("GPL");
