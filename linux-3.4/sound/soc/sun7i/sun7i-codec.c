...
static int codec_capture_linein_adc_open(void)
{
	 /*select linein source*/
	 codec_wr_control(SUN7I_ADC_ACTL, 0x7, 17, 0);
	 //boost up record effect
	 codec_wr_control(SUN7I_DAC_TUNE, 0x3,8,0x3);
	 //enable adc digital
	 codec_wr_control(SUN7I_ADC_FIFOC, 0x1,ADC_DIG_EN, 0x1);
...
static int codec_play_stop(void)
{
	...
	//pa mute
	codec_wr_control(SUN7I_DAC_ACTL, 0x1, PA_MUTE, 0x0);
	 codec_wr_control(SUN7I_ADC_ACTL, 0x7, 17, 2);
	#endif
	mdelay(5);
	//disable dac drq
...
/*
*	codec_headphone_enabled == 1,select headphone output.
*	codec_headphone_enabled == 0,
*/
static int codec_set_headphoneout(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	codec_headphone_enabled = ucontrol->value.integer.value[0];

	if (codec_headphone_enabled) {
		/*EN  MIXPAS*/
		codec_wr_control(SUN7I_DAC_ACTL, 0x1, MIXPAS, 0x1);
		/*EN PA*/
		codec_wr_control(SUN7I_ADC_ACTL, 0x1, PA_ENABLE, 0x1);
...
static int codec_get_headphoneout(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = codec_headphone_enabled;
	return 0;
}

/*
*	codec_linein == 1,select linein------mixer.
*	codec_linein == 0,
*/
...
*	codec_linein == 0,
*/
static int codec_set_linein_record(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	codec_linein_record = ucontrol->value.integer.value[0];
	return ret;
}

static int codec_get_linein_record(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = codec_linein_record;
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_phonemic_enabled) {
		codec_wr_control(SUN7I_ADC_ACTL, 0x1, ADC_SELECT, 0x2);
		/*close mic2 pa*/
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (codec_headsetmic_enabled) {
		codec_wr_control(SUN7I_ADC_ACTL, 0x1, ADC_SELECT, 0x3);
		/*close mic1 pa*/
		codec_wr_control(SUN7I_ADC_ACTL, 0x1, MIC1_EN, 0x0);
		/*enable mic2 pa*/
	 	codec_wr_control(SUN7I_ADC_ACTL, 0x1, MIC2_EN, 0x1);
	 
 		codec_wr_control(SUN7I_MIC_CRT, 0x3,mic2_preg1,0x1);
		/*enable  VMIC bias*/
		codec_wr_control(SUN7I_ADC_ACTL, 0x1, VMIC_EN, 0x1);
...
static const struct snd_kcontrol_new codec_snd_controls[] = {
	...
	*/
	...
	SOC_SINGLE_BOOL_EXT("Audio linein record", 0, codec_get_linein_record, codec_set_linein_record),
};
   ...
   if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		capture_prtd = substream->runtime->private_data;
	  	 ...
	  	 * codec <--> BT codec or GSM modem -- lg FIXME */
	   	...
	   	if (codec_linein_record){
			codec_capture_linein_adc_open();
		}else{
	   		codec_capture_open();
	   	}
	   	//set the dma
...
//begin----------------add by parker ------------open line-in

static void linein_open_entry()
{
         /*select linein source*/
         codec_wr_control(SUN7I_ADC_ACTL, 0x7, 17, 0);
         //boost up record effect
...
static void linein_close_entry()
{
       printk("-----------line-in close----------\n");
}

void open_linein(void)
{
       linein_open_entry();
}
EXPORT_SYMBOL_GPL(open_linein);

void close_linein(void)
{
       linein_close_entry();
}
EXPORT_SYMBOL_GPL(close_linein);
